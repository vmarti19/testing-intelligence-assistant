        ...
        for indicator in Indicator.items():
            for index, function in enumerate(indicator.inputs_process):
                ...
                if function and 'Flash' in function:
                    continue
                with test.step(f'{indicator.name} {indicator.color} Check the High Voltage flag case: {index}'):
                    for voltage_flag in indicator.volts:
                        if voltage_flag and 'HV' in voltage_flag:
                            ...
                            test.markdown('Set the applicable high voltage flag')
                            DUT.power_supply.flag(voltage_flag)
                            indicator.check(indicator.name, 'inactive')
                            test.markdown('Return to high voltage recovery threshold')
                            DUT.power_supply.flag(voltage_flag, 'recovery')
                            indicator.check(indicator.name, 'active', color=indicator.color)
                            indicator.deactivate()
                            indicator.disable()
                        else:
                            test.markdown(f'High voltage flag is not applicable for indicator {indicator.name}')
