...
"""
title: IND-NET-Invalid
goal: Verify feature activates or deactivates to input conditions that include an invalid state
author: jochoa5
type: Functional
feature: Indicators
function: Generic
...
start:
    - Normal operation
sequence: |-
    1. Loop for each invalid condition of the icon
        1. > Set random valid ignition status
        1. > Enable indicator randomly
        1. > Set indicator to any status different from expected at invalid
        1. > Set indicator input to invalid state
        1. ✓ Verify indicator in active or inactive state as per requirement
        ...
        for indicator in Indicator.items():
            for index, function in enumerate(indicator.inputs_process):
                ...
                if function and 'Flash' in function:
                    continue
                with test.step(f'{indicator.name} {indicator.color} Invalid case: {index + 1}'):
                    for _input in indicator.logic.select('[?fault && input[?invalid]]'):
