    ...
    def validate_file_path(self) -> Self:
        ...
        if empty:
            ...
            if isinstance(fls[0], str):
                for fl in fls:
                    if Path(fl).is_file() and os.path.splitext(fl)[1] in [".docx", ".html", ".xml", ".csv", ".pdf", ".pptx", ".xlsx", ".json", ".zip", ".txt", "", ".md", ".png", ".jpg"]:
                        files.append(fl)
            else:
                for fl in fls:
                    if fl.is_file() and os.path.splitext(fl)[1] in [".docx", ".html", ".xml", ".csv", ".pdf", ".pptx", ".xlsx", ".json", ".zip", ".txt", "", ".md", ".png", ".jpg"]:
...
class MarkItDownReader(BaseReader):
    ...
    def load_data(
        ...
        **kwargs,
        ...
        for f in tqdm.tqdm(fs, desc="Reading Docs"):
            try:
                res = self._reader.convert(f, **kwargs)
                ...
                print(f'Unable to decode file {f.__str__()}, skipping...')
